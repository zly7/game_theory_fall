...
class GamePredictor(nn.Module):
    ...
    def __init__(self,theta_init, num_local_params, count_local_params, t_hist = 7, nhidden=16, nlayers = 2, velocity = 0, use_deltav = True, deltav_plus = 1.1, deltav_minus = 0.3):  
        ...
        super().__init__()
        if nlayers == 2:
            self.lstm = nn.Sequential(nn.Linear(4*(t_hist-velocity)+velocity,int(1*nhidden)),
                                      nn.CELU(),
                                      nn.Linear(int(1*nhidden),int(1.5*nhidden)),
                                      nn.CELU(),
                                      RandMult())
        else:
...
'''
这个SubspacePredictor
本质上输出的是什么呢？ sv0, modexp(gaussp[0]), modexp(gaussp[1]) 后者是高斯分布
非常明确的是输入的是一个两辆车的历史矩阵操作;
'''
...
'''
这个本质上是一个矩阵[:2] 是other [2:] 是merge车辆
encode成5种元素
'''
