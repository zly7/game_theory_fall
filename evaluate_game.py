...
dataset_name_ssp = "highD-pretrained" # for the pretrained models either "hee-pretrained" or "highD-pretrained"
dataset_name = "highD-pretrained" # for the pretrained models either "hee-pretrained" or "highD-pretrained"
...
for train_idx, test_idx in list(kf.split(tracks))[fold:]:
    ...
    if True:    
         ...
         for i in range(25):
             ...
             sols = model(None,
                          test_games[i],
                          torch.tensor(tracks_test[i][:t_hist,:]).float(),
                          pred_test_ss[i],test_conds[i][1],
                          validate = validate,
                          verbose=game_verbose,
                          history_size = pred_size)
             
             gt = tracks_test[i][t_hist:]
             save_trajectories("/Users/zhuhe/Desktop/Game/trajectory_games_learning/predictions/%s_vdelta%d_validate%d_fold%d_predsize%d_testtrack%d.pickle" % (dataset_name,use_deltav,validate,fold,pred_size, i), sols, tracks_test[i],t_hist)
         ...
         MAE_list.append((fold_mae/fold_count)[eval_points])
         ...
         RMSE_list.append(((fold_rmsq/fold_count)**0.5)[eval_points])
...
plt.plot(range(25),MAE_list)
plt.plot(range(25),RMSE_list)
plt.show()
